...
重要：如果对话上下文中包含 [已有图谱节点] 信息，说明用户已经探索过这些模块。
生成 scope 时要包含完整的子领域列表，但对已探索过的模块加 [已探索] 前缀标记。
判断时做语义匹配——即使名称不完全相同，只要含义重叠就标记为已探索。
例如已有节点"Transformer架构"，scope中应写为"[已探索]Transformer架构原理"。
未探索的模块正常写，不加前缀。"""
